...
class ActivityAdmin(ModelView, model=Activity):
    column_list = [Activity.id, Activity.name, Activity.resource_id,Activity.thumbnail]
    name = "Activity"
...
def register(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    return crud.create_user(db, user.email, user.password, is_admin,user.name)
...
def get_leaderboard(
    ...
    leaderboard_data = (
        db.query(
            ...
            User.name.label("name"),  # Added name here
        ...
        .group_by(User.id, User.name, User.email)  # Added User.name to group_by
    ...
    formatted_leaderboard = [
        {
            "rank": idx + 1,
            "user_id": row.user_id,
            "name": row.name,  # Added name in response
            "email": row.email,
