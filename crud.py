...
def create_user(db: Session, email: str, password: str, is_admin: bool = False,name: str=None) -> User:
    """Create a new user"""
    hashed_password = hash_password(password)
    db_user = User(email=email, password=hashed_password, is_admin=is_admin,name=name)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
